...
# NN의 구조 구현
class Network(nn.Module):  # nn.module이 아닌 nn.Module로 수정
    # 생성자
...
class Dqn():
    def __init__(self, input_size, nb_action, gamma):
        ...
        self.last_action = 0
        self.last_reward = 0
        self.nb_action = nb_action  # 여기에서 nb_action을 정의합니다

    
    def select_action(self, state):
        # 현재 state로부터 action을 고를 확률 합 = 1
        probs = F.softmax(self.model(Variable(state, volatile=True).to(device)) * 100, dim=-1)
    ...
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        # batch_action 값을 유효한 범위로 제한
        batch_action = torch.clamp(batch_action, 0, self.nb_action - 1)
        
        outputs = self.model(batch_state).gather(1, batch_action.unsqueeze(1).long().to(device)).squeeze(1)
    ...
    # 실제 map에서 t시점에 받아온 정보들로 학습(memory update) 실행
    def update(self, reward, new_signal):  # 사실상 signal(리스트) = state(torch tensor)
        ...
        self.memory.push(
            (self.last_state, new_state, torch.LongTensor([int(self.last_action)]).to(device), torch.Tensor([self.last_reward]).to(device))
